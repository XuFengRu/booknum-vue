...
const isSubmittingPw = ref(false) // 防連點狀態

// 處理修改密碼
const handleUpdatePassword = async () => {
    if (isSubmittingPw.value) return;

    // 防呆：檢查是否都有填寫
    if (!pwForm.value.current || !pwForm.value.new || !pwForm.value.confirm) {
        Swal.fire({ icon: 'warning', title: '格式錯誤', text: '請確實填寫所有密碼欄位' })
        return;
    }

    // 防呆：檢查新密碼兩次是否一致
    if (pwForm.value.new !== pwForm.value.confirm) {
        Swal.fire({ icon: 'warning', title: '密碼不一致', text: '新密碼與確認密碼不相同，請重新確認', confirmButtonColor: '#f8c471' })
        return;
    }

    // 防呆：檢查新密碼強度
    const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/;
    if (!passwordRegex.test(pwForm.value.new)) {
        Swal.fire({ icon: 'warning', title: '強度不足', text: '新密碼必須至少 8 個字元，且同時包含英文字母與數字', confirmButtonColor: '#f8c471' })
        return;
    }

    // 防呆：避免新密碼跟舊密碼一樣
    if (pwForm.value.current === pwForm.value.new) {
        Swal.fire({ icon: 'info', title: '無效更改', text: '新密碼不能與目前密碼相同' })
        return;
    }
    ...
    try {
        ...
        await Swal.fire({ icon: 'success', title: '更新成功', text: response.data.message, confirmButtonColor: '#0d6efd' })
        ...
        Swal.fire({ icon: 'error', title: '更新失敗', text: errorMsg })
    } finally {
            ...
            <div class="row g-0 h-100">
                    ...
                    <div class="row g-4">
                        <div class="col-12">
                            ...
                            <div class="input-group-custom mb-0">
                                <input type="password" v-model="pwForm.current" class="form-control"
                                    placeholder="請輸入目前密碼">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group-custom mb-0">
                                <input type="password" v-model="pwForm.confirm" class="form-control"
                                    placeholder="請再次輸入新密碼">
                                <i class="bi bi-check-circle"></i>
                            </div>
                        </div>
                        <div class="col-12 mt-2">
                            <p class="text-muted small ms-2 mb-0"><i class="bi bi-info-circle me-1"></i>至少 8
                                個字元，建議包含英文和數字</p>
                        </div>
                    </div>

                    <div
                        class="d-flex justify-content-end align-items-center mt-auto pt-4 border-top border-light-subtle">
                        <button @click="handleUpdatePassword" :disabled="isSubmittingPw"
                            class="btn btn-primary rounded-pill flex-fill text-nowrap shadow-sm fs-6">
                            <span v-if="isSubmittingPw" class="spinner-border spinner-border-sm me-2" role="status"
                                aria-hidden="true"></span>
                            {{ isSubmittingPw ? '處理中...' : '更新密碼' }}
                        </button>
                    </div>
                ...
                <div class="col-lg-6 h-100 p-4 p-xl-5 bg-light d-flex flex-column">
                    ...
                    <h5 class="fw-bold text-dark mb-3"><i
                            class="bi bi-link-45deg text-primary me-2 opacity-75"></i>社群帳號連結</h5>
                    <div class="row g-3">
                        <div class="col-12">
                            <div
                                class="d-flex align-items-center justify-content-between p-3 rounded-4 bg-white border border-light-subtle shadow-sm">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div class="text-start">
                                        ...
                                        <small class="text-success fw-bold"><i
                                                class="bi bi-check-circle-fill me-1"></i>已連結</small>
                                    </div>
                                </div>
                                <button
                                    class="btn btn-danger fw-bold rounded-pill px-4 shadow-sm text-nowrap">解除</button>
                            </div>
                        </div>
                        <div class="col-12">
                            <div
                                ...
                                <button
                                    class="btn btn-outline-primary rounded-pill px-4 fw-bold text-nowrap">連結帳號</button>
                            </div>
                        </div>
                    </div>

                    <div class="border border-danger border-opacity-50 rounded-4 p-4 mt-auto bg-white shadow-sm">
                        <h6 class="fw-bold text-danger mb-3"><i class="bi bi-exclamation-triangle-fill me-2"></i>危險區域
                        </h6>
                        <p class="small text-muted mb-4">這些操作無法復原，請謹慎使用</p>

                        <div
                            class="d-flex justify-content-between align-items-center mb-4 pb-4 border-bottom border-danger border-opacity-25">
                            <div>
                                <h6 class="fw-bold mb-1 text-dark">停用帳戶</h6>
                                <small class="text-muted">暫時停用您的帳戶 (Status = 0)</small>
                            </div>
                            <button @click="handleDeactivate('disable')"
                                class="btn btn-outline-danger fw-bold rounded-pill px-4">停用</button>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button @click="handleDeactivate('delete')"
                                class="btn btn-danger fw-bold rounded-pill px-4 shadow-sm">刪除帳戶</button>
                        </div>
                    </div>
