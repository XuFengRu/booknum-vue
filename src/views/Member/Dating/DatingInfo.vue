...
const person = ref({
  ...
  hobbies: ['旅行', '攝影', '咖啡', '爬山', '閱讀', '電影', '美食'],
  intro: '喜歡旅行和攝影，熱愛探索世界的每個角落。透過鏡頭記錄不同文化、風景與人文故事，在行走中感受城市的溫度與自然的壯麗。無論是巍峨山巒、靜謐海岸，或是街頭巷尾的日常片刻，都能成為靈感來源。',
  matches: 24,
  likes: 156,
  preference: {
    gender: '女性',
...
<template>
  <div class="w-100 fade-in-up d-flex justify-content-center pb-4">
    ...
    <div v-if="!isEditing" class="card overflow-hidden border-0 shadow-lg rounded-5 w-100 mx-auto" style="max-width: 1300px; height: 650px;">
      <div class="row g-0 h-100">
        ...
        <div class="col-lg-4 h-100 p-4 p-xl-5 bg-white d-flex flex-column border-end border-light-subtle">
          ...
          <div class="row g-3 mb-4">
            <div class="col-6">
              <div class="bg-white border border-light-subtle shadow-sm rounded-4 p-3 d-flex flex-column align-items-center justify-content-center text-center h-100 transition-all">
          ...
          <div class="flex-grow-1 overflow-hidden">
            <h5 class="fw-bold text-muted mb-2"><i class="bi bi-chat-quote-fill me-2 text-primary"></i>關於我</h5>
        ...
        <div class="col-lg-4 h-100 p-4 p-xl-5 bg-light d-flex flex-column">
          ...
          <div class="mb-4">
            <h5 class="fw-bold text-muted mb-3"><i class="bi bi-stars me-2 text-warning"></i>興趣愛好</h5>
            <div class="d-flex flex-wrap gap-2">
              <span v-for="hobby in person.hobbies" :key="hobby" class="badge bg-white text-dark border border-light-subtle rounded-pill fw-normal px-3 py-2 fs-6 shadow-sm">
                {{ hobby }}
              </span>
            </div>
          </div>
          ...
          <div class="mb-4 flex-grow-1">
            <h5 class="fw-bold text-muted mb-3"><i class="bi bi-balloon-heart-fill me-2 text-danger"></i>交友偏好</h5>
            <div class="bg-white rounded-4 p-4 shadow-sm border border-light-subtle">
              ...
              <div class="mb-3">
                ...
                <div class="fw-bold text-dark">{{ person.preference.ageRange[0] }} - {{ person.preference.ageRange[1] }} 歲</div>
              </div>
              <div>
                <div class="small text-muted mb-1">接受地區</div>
                <div class="fw-bold text-dark lh-sm">
                  {{ person.preference.cities.length > 0 ? person.preference.cities.join(', ') : '不拘' }}
                </div>
              </div>
            </div>
          </div>
    ...
    <DatingEdit v-else :initial-data="person" @save="updateProfile" @cancel="isEditing = false" />
