...
const API_BASE_RAW = import.meta.env.VITE_API_BASE_URL || "https://localhost:7091";
const API_BASE = String(API_BASE_RAW).replace(/\/+$/, "");

// ✅ 跟列表頁統一（務必是資料庫存在的 userId）
const CURRENT_USER = { id: 1, name: "Arille.M" };

/** ✅ 跟列表頁共用 localStorage Key（同步用） */
...
function toAbsImageUrl(imgOrUrl) {
  const v = String(imgOrUrl ?? "").trim();
  if (!v) return "";
...
function isFull(a) {
  return (Number(a.joinedCount) || 0) >= (Number(a.need) || 0);
}
...
async function fetchActivityDetail() {
  ...
  loading.value = true;
  try {
    if (!id.value) throw new Error("活動 ID 不正確");

    // ✅ 一定帶 userId，後端才會回 isJoinedByMe
    const res = await axios.get(`${API_BASE}/api/activities/${id.value}`, {
      params: { userId: CURRENT_USER.id },
    });

    const data = res.data || {};

    const aid = Number(data.activityId ?? data.ActivityId ?? 0);
    ...
    const isJoinedByMe = Boolean(data.isJoinedByMe ?? false);

    /** ✅ 新增：抓 ownerId（跟列表頁同一套欄位猜測） */
    ...
    activity.value = {
      ...
      image: toAbsImageUrl(data.imageUrl ?? data.ImageUrl ?? data.image ?? data.Image),
    };

    // ✅ 同步 localStorage（讓列表頁可以靠 onActivated refreshPage 同步）
    const s = loadJoinedIds();
    if (activity.value.isJoinedByMe) s.add(aid);
    else s.delete(aid);
    saveJoinedIds(s);
...
const remaining = computed(() => {
  ...
  if (!a) return 0;
  return Math.max(0, (Number(a.need) || 0) - (Number(a.joinedCount) || 0));
});
...
const progressPct = computed(() => {
  ...
  const joined = Math.max(0, Number(a.joinedCount) || 0);
...
async function toggleJoin() {
  ...
  try {
    ...
    if (a.isJoinedByMe) {
      ...
      await axios.delete(`${API_BASE}/api/activities/${aid}/join`, {
        params: { userId: uid },
      });

      // ✅ 同步 localStorage（列表頁會跟著同步）
      const s = loadJoinedIds();
      s.delete(aid);
      saveJoinedIds(s);

      alert("已取消報名");
      await fetchActivityDetail(); // ✅ 立即同步人數/按鈕
      return;
    }
    ...
    const res = await axios.post(`${API_BASE}/api/activities/${aid}/join`, null, {
  params: { userId: uid },
});

// 不管 Google 成功或失敗都只顯示成功
alert("報名成功！");
    ...
    if (status === 400) alert(msg || "操作失敗");
    else if (status === 409) alert("活動已額滿");
    else alert("操作失敗，請查看 Console / Network");
  }
}

function goBack() { router.back(); }
function goList() { router.push({ name: "member-group" }); }
                ...
                <div class="p-4 d-flex flex-column">
                  ...
                  <div
                    ...
                    <span class="fw-bolder fs-6" :class="isFull(activity) ? 'text-danger' : 'text-primary'">
                      {{ activity.joinedCount }} / {{ activity.need }} 人
                    </span>
                  </div>
            ...
            <div class="flex-grow-1 d-flex flex-column">
              ...
              <div
                ...
                <div class="text-muted small fw-bold d-none d-md-block">
                  ...
                  <span v-else-if="isMyOwnActivity(activity)"><i
                      class="bi bi-exclamation-circle-fill text-secondary me-1"></i>不能報名自己發布的活動</span>
                  ...
                  <button class="btn rounded-pill px-5 fw-bold shadow-sm flex-grow-1 text-nowrap" :class="isStarted(activity)
                      ...
                      : (activity.isJoinedByMe
                          ...
                          : ((isMyOwnActivity(activity) || isFull(activity))
                              ? 'btn-secondary text-white border-0 opacity-50'
                              : 'btn-primary'))
                    " :disabled="isStarted(activity) || (!activity.isJoinedByMe && (isFull(activity) || isMyOwnActivity(activity)))"
                    @click="toggleJoin">
                    {{ isStarted(activity) ? "活動已開始"
                      : (activity.isJoinedByMe ? "取消報名"
                        : (isMyOwnActivity(activity) ? "自己發布"
                          : (isFull(activity) ? "已額滿" : "立即報名"))) }}
                    <i v-if="!isStarted(activity) && !activity.isJoinedByMe && !isFull(activity) && !isMyOwnActivity(activity)"
...
.glass-shell {
  ...
  box-shadow: 0 18px 60px rgba(0, 0, 0, .10), inset 0 1px 0 rgba(255, 255, 255, .55);
  overflow: hidden;
}
.glass-side { background: rgba(255, 255, 255, .22); border-right: 1px solid rgba(255, 255, 255, .45); }
.glass-main { background: rgba(255, 255, 255, .12); }
.glass-card {
  border: 1px solid rgba(255, 255, 255, .55);
