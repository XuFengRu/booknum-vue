...
const UserName = ref(storedUser.name || '會員')
const UserPhoto = ref(storedUser.photo || null)
const hasInfo = ref(storedUser.hasInfo || false)
// 登出
const handleLogout = () => {
  Swal.fire({
    title: '確定要登出嗎？',
  ...
  <div class="member-page">
    ...
    <aside class="offcanvas-lg offcanvas-start" :class="{ 'show': isSidebarOpen }" id="sidebar">
      ...
      <nav class="nav flex-column flex-grow-1">
        <RouterLink v-for="(item, index) in currentMenu" :key="index" :to="item.link" class="nav-link mb-2"
          active-class="active" @click="closeSidebar">
    ...
    <main class="main-layout">
      <header class="sticky-header d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center gap-3">
          ...
          <div class="dropdown">
            <button
              class="bg-white bg-opacity-75 border border-white shadow-sm rounded-pill d-flex align-items-center gap-2 ps-3 pe-1 py-1 transition-all"
              data-bs-toggle="dropdown" style="backdrop-filter: blur(10px);">
              <span class="fw-bold text-dark small text-nowrap">{{ UserName }}</span>
              <img v-if="UserPhoto" :src="UserPhoto" class="rounded-circle border border-2 border-white bg-white"
                width="36" height="36" style="object-fit: cover;" alt="User">
              <div v-else
                class="rounded-circle border border-2 border-white bg-light d-flex justify-content-center align-items-center text-secondary"
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 rounded-4 mt-2">
              <li>
                <RouterLink class="dropdown-item py-2" to="/member/profile"><i class="bi bi-person me-2"></i>個人資料
                </RouterLink>
              </li>
              <li>
                <RouterLink class="dropdown-item py-2" to="/member/security"><i class="bi bi-shield-lock me-2"></i>帳號安全
                </RouterLink>
              </li>
              <li>
