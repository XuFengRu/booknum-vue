...
const hobbyOptions = [
  "旅行","攝影","咖啡","爬山","閱讀","音樂","電影","健身","程式","美食","看展","潛水","烹飪","桌遊"
]

const form = ref({
  name: "", location: "", job: "", avatar: "", hobbies: [], intro: "", // 移除了 age
  preference: { gender: "不拘", ageRange: [18, 80], cities: [] }
})
...
function submitForm() {
  if (!form.value.name || !form.value.location || !form.value.job) {
    alert("請填寫必填欄位！")
    return
  }
  ...
  <div class="card overflow-hidden border-0 shadow-lg rounded-5 w-100 mx-auto" style="max-width: 1300px; height: 650px;">
    <div class="row g-0 h-100">
      ...
      <div class="col-lg-4 h-100 p-4 p-xl-5 bg-white d-flex flex-column border-end border-light-subtle">
        <h4 class="fw-bolder text-dark mb-4 text-gradient"><i class="bi bi-pencil-square  me-2"></i>基本資料</h4>
        ...
        <div class="mb-3 flex-grow-1 d-flex flex-column">
          ...
          <textarea v-model="form.intro" class="form-control flex-grow-1" maxlength="250" placeholder="介紹一下自己吧..."></textarea>
        </div>
      </div>
      ...
      <div class="col-lg-4 h-100 p-4 p-xl-5 bg-light d-flex flex-column">
        ...
        <div class="flex-grow-1">
          <h4 class="fw-bolder text-dark mb-4 text-gradient"><i class="bi bi-sliders me-2"></i>偏好與興趣</h4>

          <div class="mb-4">
            <label class="form-label fw-bold text-muted small">興趣愛好 (最多 8 個)</label>
            <el-select v-model="form.hobbies" multiple collapse-tags placeholder="選擇興趣" style="width: 100%;" :max-collapse-tags="5" size="large">
              <el-option v-for="hobby in hobbyOptions" :key="hobby" :label="hobby" :value="hobby" 
          ...
          <div class="mb-4 px-2">
            <label class="form-label fw-bold text-muted small mb-0">期望年齡 ({{ form.preference.ageRange[0] }} ~ {{ form.preference.ageRange[1] }} 歲)</label>
            <el-slider v-model="form.preference.ageRange" range :min="18" :max="80" :step="1" />
          </div>

          <div class="mb-4">
            <label class="form-label fw-bold text-muted small">期望所在地區</label>
            <el-select v-model="form.preference.cities" multiple collapse-tags placeholder="選擇城市 (留空為不拘)" style="width: 100%;" :max-collapse-tags="5" size="large">
              <el-option v-for="city in cities" :key="city" :label="city" :value="city" />
            </el-select>
          </div>
        </div>
...
<style scoped>
:deep(.el-slider__bar) { background: var(--bs-primary); }
:deep(.el-slider__button) { border-color: var(--bs-primary); }
